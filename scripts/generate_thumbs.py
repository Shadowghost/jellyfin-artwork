...
def update_entry(entry: dict, slot: str) -> None:
    ...
    entry["template"] = True
    # `placeholder` and `template` are mutually exclusive: once we've
    # produced real templated artwork, the entry is no longer a stub.
...
# wipe out bespoke files. The aria-label always ends in ``(thumb)`` or
# ``(primary)`` for files emitted by this script. Hand-crafted thumbs
# have neither marker and stay protected. The studio name appears
# verbatim inside the aria-label so a stray match in unrelated XML is
# unlikely.
GENERATOR_SIGNATURE_RE = re.compile(
    r'\brole\s*=\s*["\']img["\'][^>]*'
    r'\baria-label\s*=\s*["\'][^"\']*\((?:thumb|primary)\)["\']',
    re.IGNORECASE,
)
...
def needs_processing(
    ...
    if entry.get("placeholder"):
        # No real logo yet - skip silently. --force overrides this for
        # entries whose placeholder flag is stale (logo was added but the
        # flag wasn't cleared, e.g. apple-corps).
        return False
...
def main() -> int:
    ...
    ap.add_argument("--aspect", choices=sorted(SPECS), action="append",
                    default=None,
                    help="Output aspect (repeatable). Default: generate "
                         "both 1x1 and 16x9.")
    ...
    aspects = list(dict.fromkeys(args.aspect)) if args.aspect else ["1x1", "16x9"]
    ...
    for aspect in aspects:
        ...
        print(f"aspect:          {aspect}  ({spec['w']}x{spec['h']}, "
